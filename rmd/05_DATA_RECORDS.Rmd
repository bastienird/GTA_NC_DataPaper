...
```{r tab.cap = "Datasets and pre-processing functions used in the creation of the final dataset", error=TRUE}
...
nominal_files_recap <- googlesheets4::read_sheet(nominal_files_url, sheet = "Nominal catches")
...
```{r nominal-files-recap, tab.cap = "Recap of the files and their associated process used for the creation of the final output 'global_nominal_catch_firms_level0', dplyr::select(-c(Description, Provenance)", results='asis', eval=TRUE}

qflextable2(nominal_files_recap %>% dplyr::select(-c(Description, Provenance)))
...
- The data is provided by year, from `r min(NC$year)` to `r max(NC$year)`.
...
mapping_files <- googlesheets4::read_sheet(link_for_mapping_files, sheet = "2023")
...
All the dataframes used for the mapping and the metadata (Dublin Core) included are to find in this link `link_for_mapping_files`.
...
```{r species-level-0, tab.cap = "Species retained in the final dataset by tRFMOs (only head)", eval = TRUE}

cl_species_level0 <- head(readr::read_csv(file_url))
