...
# construction du flextable rÃ©duit
ft_recap <- 
...
The FSJ dataset is much more exhaustive for species and flags. A greater number of species can be observed in FSJ, 347 species, while GTA lists only 65. The fishing_fleet categories are also more numerous in FSJ, due to the higher level of detail, whereas in GTA some data are aggregated (for example, 'RÃ©union' is included in France).
...
<!-- On commente le reste car peu utile c'est un peu les redites des donnÃ©es prÃ©cÃ©dentes -->
...
# 2. CrÃ©ation du tableau complet (including absences)
...
# 1. Combinaison et agrÃ©gation des valeurs de capture par source
...
# 2. Construction du tableau complet (prÃ©sence + valeurs)
...
# 3. TracÃ© de la heatmap avec comparaison des valeurs
...
# ggplot2::scale_fill_manual(
#   values = c(
#     "Both: GTA > FSJ" = "#1B9E77",  # vert bleute fonce
#     "GTA only"        = "#66C2A5",  # vert bleute clair (même teinte)
#     
#     "Both: FSJ > GTA" = "#D95F02",  # orange fonce (ou rouge brun)
#     "FSJ only"        = "#FC8D62",  # orange clair (même teinte)
#     
...
We observe the same differences as before for Skipjack, Swordfish, and Pacific Bluefin tunaâ€”which is expected, as these species occur in all oceans and are therefore not affected by the new filtering criteria. However, even after excluding Pacific data for Kawakawa and sharks, the FSJ dataset still reports higher values. This suggests that records for these species may be inaccurate for GTA.
